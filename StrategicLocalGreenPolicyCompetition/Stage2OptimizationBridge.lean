...
holding the rival post-investment intercept fixed at `vj`. -/
...
/-- Scalar reduced profit on the active-duopoly branch, up to the policy-only
constant `s^2/(2kg)`. -/
...
/-- Generic quadratic and its algebraic derivative. -/
def quadraticValue (A B C u : ℝ) : ℝ := A * u ^ 2 + B * u + C

def quadraticGradient (A B u : ℝ) : ℝ := 2 * A * u + B

/-- For a strictly concave quadratic, a zero first-order condition is a global maximum. -/
...
/-- A nonnegative derivative at the right endpoint maximizes a strictly concave
quadratic over all points to its left. -/
...
/-- A nonpositive derivative at the left endpoint maximizes a strictly concave
quadratic over all points to its right. -/
...
/-- The displayed active-duopoly FOC expression is exactly `2 A u + B`. -/
...
/-- The displayed monopoly FOC expression is exactly `2 A u + B`. -/
...
/-- A zero active-duopoly FOC gives the branch-global maximum. -/
...
/-- A zero monopoly FOC gives the branch-global maximum. -/
...
theorem model_aMonopoly_scalar_foc
  ...
  have hMne : 2 - investmentR kx kg μ ≠ 0 := by
    simpa [monopolyM] using hMpos.ne'
  unfold monopolyM
  field_simp [hRpos.ne', hMne]
  ring
...
/-- At an A-kink candidate the duopoly-side derivative is nonnegative and the
monopoly-side derivative is nonpositive. -/
